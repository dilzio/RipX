...
public class OneForOneRestartPolicy implements RestartPolicy {
	...
	private static final Logger LOG = LoggerFactory.getLogger(OneForOneRestartPolicy.class.getName());
	...
	public void apply(final RunnableWrapper rw) {
		...
		if (lastRestartWasInWindow(System.currentTimeMillis(), lastRestartTime)) {
			if (counterForRunnable.get() < _maxRestartsInWindow) {
				...
				LOG.error("Exceeded max restarts for runnable " +  rw.getName());
			}
		}else{
	...
	private void respawnRunnable(final RunnableWrapper rw){
			_execService.execute(rw);
			LOG.warn("Respawned runnable " + rw.getName() + " on new thread");
	}
